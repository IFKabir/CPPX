...
template <typename T> void BinaryTree<T>::set_left(Node<T> *parent, std::unique_ptr<Node<T>> left_child)
{
    if (parent == nullptr)
        return;
...
template <typename T> void BinaryTree<T>::set_right(Node<T> *parent, std::unique_ptr<Node<T>> right_child)
{
    if (parent == nullptr)
        return;
...
template <typename T>
std::unique_ptr<Node<T>> BinaryTree<T>::make_node(const T &val, std::unique_ptr<Node<T>> left,
